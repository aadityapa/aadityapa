...
- **ASCII portrait pipeline**: `photo_to_ascii.py` reads `assets/source-portrait.png` (three-quarter professional headshot), flood-fills the warm-gray studio background, equalizes contrast within the subject mask, applies a 70-level density ramp (space through block glyphs; see `RAMP` in `photo_to_ascii.py`) with gamma/edge emphasis, and emits `ascii_face.py`, which `generate_svgs.py` imports. Grid: 56×29 cells. Regenerate the portrait by re-running the pipeline — never hand-edit the grid.
